...
#' @return SpatRaster with a layer for every depth
#' @export
romshcoords <- function(x, grid_type = "rho", slice, ..., depth = "h", simple = FALSE) {
  ...
  if (simple) {
    ...
    a <- array(rep(rev(Cs_r), each = length(v)) * v,
               c(ncol(h), nrow(h), length(Cs_r)))
    lyrs <- vector("list", dim(a)[3])
    for (k in seq_along(lyrs)) {
      lyrs[[k]] <- terra::rast(t(a[, , k]))
    }
    out <- set_indextent(terra::rast(lyrs))
  } else {
    ...
    zeta <- if (missing(slice)) 0 else stop("not coded yet")
    ...
    ## build multilayer SpatRaster from 3D array z[Lp, Mp, N]
    ## terra::rast() can take a 3D array but needs correct dimension interpretation
    ## z is [x, y, depth] from ncdf4; we need to transpose to [y, x] per layer
    lyrs <- vector("list", dim(z)[3])
    for (k in seq_along(lyrs)) {
      lyrs[[k]] <- terra::rast(t(z[, , k]))
    }
    out <- terra::rast(lyrs)
    out <- terra::flip(set_indextent(out), direction = "vertical")
    ## reverse layer order so surface is first
    out <- out[[rev(seq_len(terra::nlyr(out)))]]
  }
