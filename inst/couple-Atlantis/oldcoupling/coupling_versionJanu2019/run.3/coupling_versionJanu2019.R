...
# saveRDS(file_db, file = "inst/couple-Atlantis/coupling_versionJanu2019/run.3/file_db.rds")
...
for (itime in seq_len(nrow(file_db))) {
  ...
  for (ilayer in seq_len(nrow(temperature))) {
    ...
    mass_cells$vertical <- raster::extract(w0[[ilayer]], mass_cells$cell_)
    ...
    if (FALSE) {
      ...
      # Calculate the mean u and v
      mean.u <- mean(unlist(u_sub[pol]), na.rm=T)
      mean.v <- mean(unlist(v_sub[pol]), na.rm=T)
      if (is.na(mean.u | mean.v)) {
        mean.u <- 0
        mean.v <- 0
      }
      
      # Calculate angle for the face and the velocity vectors
      angle_sec[j,km,d]=(180*atan2(x[2]-x[1],y[2]-y[1]))/pi   # angle in radiants of tan(y/x) / pi resulting in angle in degrees
      angle_vel[j,km,d]=(180*atan2(mean.u,mean.v))/pi
      # Determine if flux is positive (left to right) or negative (right to left) looking from xy[1,] to xy[2,]
      # if relative angle between the line and the water direction is 0:180 the flow is from left to right = positive
      # if relative angle between the line and the water direction is -0:-180 the flow is negative (from r to left)
      relative_angle <- angle_vel[j,km,d]-angle_sec[j,km,d]
      if (relative_angle > 180 ) {  #these first two if's correct for angles higher or lower than 180/-180
        rel_angle=relative_angle-360
      } else if (relative_angle < -180) {
        rel_angle=relative_angle + 360
      } else {
        rel_angle=relative_angle
      }
      # Calculate velocity over the face (flow is positive to the left, looking from point 1 to point 2)
      vel_section[j,km,d]=sqrt(mean.u^2+mean.v^2)
      vel_section[j,km,d] <- vel_section[j,km,d]*ifelse(!is.na(angle_vel[j,km,d]) & rel_angle < 0, 1, -1)
